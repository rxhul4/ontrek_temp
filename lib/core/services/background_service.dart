    ...
    onInternetStatusChange: (status) async {
      ...
      if (isInternetAvailable == true) {
        ...
        PreferenceHelper.setDouble(PreferenceHelper.LAST_INTERNET_OFF_LAT,
            currentLatitude ?? (prevLatitude ?? 0.0));
        PreferenceHelper.setDouble(PreferenceHelper.LAST_INTERNET_OFF_LONG,
            currentLongitude ?? (prevLongitude ?? 0.0));
        ...
        try {
          throttlerOfflineDataAdd.run(() async {
            ...
            if (lastActivityData != null) {
              ...
              if (physicalActivity != "ActivityType.STILL" ||
                  physicalActivity == "") {
                await ManageRouteHistory();
              }
...
ManageInternetOperations() async {
  ...
  if (internetOffLat == null ||
      internetOffLat == 0 && internetOffLat == 0.0 ||
      internetOffLong == null ||
      internetOffLong == 0.0 && internetOffLong == 0) {
    Position position = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.best);
  ...
  var internetOffActivity = Activity(
      ...
      longitude:
          internetOffLong ?? (currentLongitude ?? (prevLongitude ?? 0.0)),
  ...
  bool isDifferenceLessTwoMinutes = await isDifferenceLessFiveMinutes(
      internetOffActivity.activityDate ?? "",
      internetOnActivity.activityDate ?? "");
...
setLastActivityData() async {
  ...
  Map<String, dynamic> lastActivityMap =
      PreferenceHelper.getObject(PreferenceHelper.LastActivity);
